    ...
    def is_uid_authorized_to_join(bot_uid, target_uid):
        ...
        try:
            ...
            if os.path.exists(member_path):
                ...
                # [সিনট্যাক্স এরর ফ্রি ডাইনামিক ডাটা এক্সট্রাক্টর]
                all_uids_flat = []
                
                # গিল্ড লিডার
                leader_obj = guild_data.get("leader")
                if leader_obj: 
                    all_uids_flat.append(extract_uid(leader_obj))
                
                # একটিং লিডার
                al_obj = guild_data.get("acting_leader")
                if al_obj: 
                    all_uids_flat.append(extract_uid(al_obj))
                
                # ক্ল্যান অফিসার্স
                for off in guild_data.get("officers", []):
                    all_uids_flat.append(extract_uid(off))
                    
                # ক্ল্যান সাধারণ মেম্বার্স
                for mem in guild_data.get("members", []):
                    all_uids_flat.append(extract_uid(mem))
                    
                # আইডি ক্লিয়ারেন্স ও ফিল্টারিং
                all_uids_flat = [x for x in all_uids_flat if x and x != "None" and x != "0" and x != ""]
